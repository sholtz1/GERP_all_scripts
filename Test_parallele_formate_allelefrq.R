...
parellel_allele_BP <- do.call(rbind, clusterApplyLB(clust, 1:nrow(Fixed_only), function(i){
  ...
  # calculations and code go here...
    temp <- names(Fixed_only_bases)[which(Fixed_only_bases[i,] == Fixed_only$alternate[i])]
    
  return(data.frame(Fixed = temp))   # this is the end product for each iteration of the loop
}))


Fixed_only$Fixed <- parellel_allele_BP$Fixed

t1_parallel <- proc.time() -ptm
...
parellel_minor_BP <- do.call(rbind, clusterApplyLB(clust, 1:nrow(FRQ_test), function(i){
  ...
  bases2 <- bases[1]
  return(data.frame(Which_base = bases2))   # this is the end product for each iteration of the loop
}))


FRQ_test$New <- parellel_minor_BP$Which_base
...
#tb <- names(Base_only)[which(Base_only[i,] == FRQ_test$Minor[i])]
#the_base <- tb[1]
#  FRQ_test$New[i] <- the_base
#}
...
# For some reason replacing a colum of NAs results in a diffierent result
# then replacing a column that already has data but all number should be replaced 
# either way
#identical(Allele_frequencies$alternate2, parellel_Allele_frqs_name$Alternate)
##Normal loop

#Allele_frequencies$alternate2 <- rep(NA, nrow(Allele_frequencies))

#for (i in 1:nrow(Allele_frequencies)) {
#  Allele_frequencies$alternate2[i] <- Allele_frequencies[i, which(names(Allele_frequencies) == Allele_frequencies$New[i])]
#  
