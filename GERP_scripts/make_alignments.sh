...
last_path=./GERPsoftware/last/src/
...
$last_path/last-train -P0 --revsym --matsym --gapsym -E0.05 -C2 $db_prefix $query > $mat

mkdir -p ./analyses/last/maf/

maf=./analyses/last/maf/Tribolium_ref_2020_"$query_name".maf

$last_path/lastal -m50 -E0.05 -C2 -l25  -p $mat $db_prefix $query > $maf
...
$last_path/maf-convert axt $maf > $axt
...
$last_path/last-split -m1 "$net_maf"_w_header |
$last_path/maf-swap |
awk -v q="$query_name" -v r="$ref_name" '/^s/ {$2 = (++s % 2 ? q "." : r ".") \
$2} 1' | $last_path/last-split -m1 | \
$last_path/maf-swap | $last_path/last-postmask > $one_maf
