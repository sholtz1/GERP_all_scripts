...
run Rscript Format_allele_frqs_parallel.R 
This script takes the number of neucleotides in each pooled genetic sample turns this into allele frequencies
Final file "Filtered_FRQs_para.delim"

### Combine with GERP scores ####

starting file "Filtered_FRQs_para.delim"
run Rscript combine_all.R
This sctips joins the GERP data with the allele frequency data based on chromosome and location
Final file "Full_data_bigtree.delim"
...
Fourth is the amino acid cahnge
...
starting files "Full_data_bigtree.delim" and oupult file from Ensembl VEP
run Rscript Variant_effects.R
This will take the amino acid changes identified by Ensembl VEP for each loactation in the genome and create a grantham score for it
It will then use those grantham scores to calculate a measure of genetic load for each Landscape and treatment.
Final file "Grantham_loads.delim"
...
Starting files "Beetle_fitness_filtered.delim", "Full_data_bigtree.delim", and "Grantham_loads.delim"
Run Rscript formatting_final_summary.R
This script takes all of the GERP and allele frequency information and caclculates diffierent summary stats
for each landscape and treatment and attaches that to our fitness data. This data can then be used to compare population lelvel
genetic loads and allele frequency changes to the fitnesses of populations
Final file "Final_summary.delim"


###### generate a final model and create figures ##########
final model fitting and figure creation are all done in R using the Rscript final_analyses.R
